...
const Web3Watcher = () => {
  ...
  let stateHook = (state) => {
    throw new Error('Web3Watcher stateHook not set')
  }
  ...
  const onAccountsChanged = (accounts) => {
    $.accounts = accounts
    updateState({})
    typeof eventHandler.accountsChanged === 'function' &&
      eventHandler.accountsChanged($)
  }

  const onChainChanged = (networkId) => {
    $.networkId = networkId
    updateState({})
    typeof eventHandler.chainChanged === 'function' &&
      eventHandler.chainChanged($)
  }

  $.reInit = async (options) => {
    if (window.ethereum) {
  ...
  $.init = async (options) => {
    ...
    typeof eventHandler.connected === 'function' &&
      $.state.isConnected &&
  ...
  $.getWalletType = () =>
    new Promise((resolve, reject) => {
      if (!$.instance || !$.instance.currentProvider)
        return reject(new Error('can determine provider'))
